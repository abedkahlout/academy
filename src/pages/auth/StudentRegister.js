...
export default function StudentRegister() {
  ...
  const test = useSelector((state) => state.super.superSlice);
	...
	useEffect(() => {
		...
		fetch(`${process.env.REACT_APP_API}/api/privateSchool/all`,
    {
      headers: {
        Authorization: test?.token,
			...
			.then((res) => res.json())
			.then((data) => setSchool(data.privateSchools));
	}, []);
		...
		<div className="student-register container">
			<div className="register-top">
				...
				<div className="register-as">
					...
					<Link to={"/register/parent"} className="notActive">
						تسجيل كأب{" "}
					</Link>
				</div>
				<div className="register-process-wrapper">
					<form onSubmit={handleSubmit(onSubmit)} className="register-form">
						...
						<div className="form-input-wrapper">
							...
							<span style={{ color: "red" }}>{errors.LevelId?.message}</span>
						</div>
						<div className="form-input-wrapper">
							<label className="input-title"> السنة الدراسية </label>
							<select
								{...register("ClassId", { required: " السنة الدراسية مطلوب" })}
								className="input select"
								onChange={(e) => {
                ...
                > */}
							...
							<span style={{ color: "red" }}>{errors.ClassId?.message}</span>
						</div>
						<div className="form-input-wrapper">
                ...
                > */}
							...
							<span style={{ color: "red" }}>{errors.SectionId?.message}</span>
						</div>
						<div className="form-input-wrapper">
							<label className="input-title">المدرسة الخاصة</label>
							<select {...register("SchoolID")} className="input select">
								<option value={""} disabled selected>
									اختر المدرسة
								</option>
								{school?.map((e, i) => (
									<option className="option" value={e.id}>{e.name}</option>
								))}
							</select>
							{/* <span style={{ color: "red" }}>{errors.school?.message}</span> */}
						</div>
